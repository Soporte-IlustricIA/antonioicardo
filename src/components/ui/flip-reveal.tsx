    ...
    useGSAP(
        () => {
            ...
            items.forEach((item) => {
                ...
                if (isShow(key)) {
                    if (showClass) item.classList.add(...showClass.split(" ").filter(Boolean));
                    if (hideClass) item.classList.remove(...hideClass.split(" ").filter(Boolean));
                } else {
                    if (showClass) item.classList.remove(...showClass.split(" ").filter(Boolean));
                    if (hideClass) item.classList.add(...hideClass.split(" ").filter(Boolean));
                }
            });
